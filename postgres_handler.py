...
def get_within_time(connection, org, days=7):
    ...
    cursor.execute(
        "SELECT url FROM articles WHERE org=(%s) AND last_modified>(%s)",
        (org,
         timestamp))
...
class ArticleHandler:
    ...
    def get_article(self, url):
        self.cursor.execute(
            "SELECT url, data FROM articles WHERE url=(%s)", (url,))
        return self.cursor.fetchone()

    def update_article(self, url, data):
        self.cursor.execute(
            "UPDATE articles SET data=(%s), last_modified=(%s) WHERE url=(%s)",
            (Json(data),
    ...
    def create_article(self, url, org, data):
        self.cursor.execute(
            "INSERT INTO articles (url, org, last_modified, data) VALUES (%s,%s,%s,%s)",
            (url,
             org,
             datetime.datetime.now(),
             Json(data)))
