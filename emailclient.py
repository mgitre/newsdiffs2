...
class CustomEmail:
    ...
    def addArticle(self, changed):
        ...
        self.content += "<a href=\"{url}\">{headline}</a> has now been <a href=\"{changedurl}\">changed</a> {times} time(s). <br><br>".format(url=url, headline=headline, times=times-1, changedurl = "http://192.168.1.10:3301/"+name+"/"+url)
    def send(self):
