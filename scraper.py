...
# establishes base methods for each site, making it fairly easy to add new
# websites to scrape
class BaseScraper:
    def __init__(self):
        pass
    # goes to homepage of website and finds all urls that match the given
    # regex pattern
    ...
    def addToData(self, article, articledata):
        ...
        if article_handler.exists:
            ...
            for key in articledata:
                if articledata[key] is None:
                    articledata[key] = latest_entry[key]
                    continue
                if latest_entry[key] is None:
                ...
                if articledata[key] != latest_entry[key]:
                    has_changed = True
            if has_changed:
                in_db.update({datetime.now().strftime(
                    "%m/%d/%Y, %H:%M:%S"): articledata})
                ...
                in_db[datetime.now().strftime("%m/%d/%Y, %H:%M:%S")
                      ] = in_db.pop(latest_in_db)
                article_handler.update_article(article, in_db)

        else:
            article_handler.create_article(article, self.name, {
                                           datetime.now().strftime("%m/%d/%Y, %H:%M:%S"): articledata})
        ...
        try:
            ...
            for name, attrs in self.bylineMatches:
                ...
                if matchAttempt:
                    ...
                    for hidden in bylinesoup.find_all(
                            attrs={"class": "hidden"}):
                        hidden.decompose()
            ...
            return url, {"headline": headline, "subheadline": subheadline,
                         "byline": byline, "body": content}
...
class WashingtonPost(BaseScraper):
    def __init__(self):
        ...
        self.headlineMatches = [
            ("h1", {"data-qa": "headline"}), ("h1", {"class": "title"}), ("h1", {"itemprop": "headline"})]
        self.subheadlineMatches = [
            ("h2", {"data-pb-field": "subheadlines.basic"}), ("h2", {"data-qa": "subheadline"})]
        self.bylineMatches = [
            ("div", {"class": "author-names"}), ("div", {"class": "contributor"})]
        self.articlebodyMatches = [("div", {"class": "article-body"}), ("article", {
            "data-qa": "main"}), ("div", {"class": "main"})]
...
class NewYorkTimes(BaseScraper):
    def __init__(self):
        ...
        self.headlineMatches = [("h1", {"itemprop": "headline"}), ("h1", {
            "data-testid": "headline"}), ("h1", {"class": "edye5kn2"})]
        self.subheadlineMatches = [
            ("p", {"id": "article-summary"}), ("p", {"class": "css-h99hf"})]
        self.bylineMatches = [
            ("p", {"itemprop": "author"}), ("div", {"class": "css-vp77d3"})]
        self.articlebodyMatches = [
            ("section", {"itemprop": "articleBody"}), ("section", {"name": "articleBody"})]
        BaseScraper.__init__(self)
